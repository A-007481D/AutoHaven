...
$totalVehicles = count($vehicle->getAllVehicles());
$totalPages = ceil($totalVehicles / $vehiclesPerPage);

$paginatedVehicles = $vehicle->getActiveVehicles($vehiclesPerPage, $offset);
        ...
        <div class="max-w-7xl mx-auto px-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                <?php foreach ($paginatedVehicles as $vehicle): ?>
                    <div class="group rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 cursor-pointer"
                        ...
                        <div class="p-6 bg-white">
                            <div class="flex justify-between items-start mb-4">
                                <div>
                                    <h3 class="text-xl font-bold text-gray-900"><?php echo htmlspecialchars($vehicle['brand'] . ' ' . $vehicle['model']); ?></h3>
                                    <!-- <p class="text-gray-500"><?php echo htmlspecialchars($vehicle['catName']); ?></p>  -->
                                </div>
                            </div>
                            <div class="flex items-center gap-4 text-sm text-gray-500 mb-4">
                                <span>🛢️ <?php echo htmlspecialchars($vehicle['fuel']); ?> </span>
                                <span>🪑 <?php echo htmlspecialchars($vehicle['seats']); ?> Seats</span>
                                <span>🚪 <?php echo htmlspecialchars($vehicle['doors']); ?> Doors</span>
                            </div>
                            <p class="text-gray-600 mb-6"><?php echo htmlspecialchars($vehicle['description']); ?></p>
                    ...
                    <div id="modal-<?php echo $vehicle['vehicleID']; ?>" class="modal hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50">
                        <div class="bg-white rounded-lg p-6 max-w-md">
                            ...
                            <ul class="text-sm text-gray-500 space-y-2">
                                <li>Fuel Type: <?php echo htmlspecialchars($vehicle['fuel']); ?></li>
                                <li>Seats: <?php echo htmlspecialchars($vehicle['seats']); ?></li>
                                <li>Doors: <?php echo htmlspecialchars($vehicle['doors']); ?></li>
                            </ul>
                            <button id="bookNow" class="w-full bg-blue-600 text-white py-3 rounded-xl hover:bg-blue-700 transition-colors">
                                Book Now
                            </button>
                        </div>
                    </div>
            ...
            <div class="flex justify-center items-center mt-8">
                <?php if ($page > 1): ?>
                    <a href="?page=<?php echo $page - 1; ?>" class="px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600 mx-1">Previous</a>
                <?php endif; ?>

                <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                    <a href="?page=<?php echo $i; ?>" class="px-4 py-2 <?php echo $i === $page ? 'bg-purple-900' : 'bg-purple-500'; ?> text-white rounded-lg hover:bg-purple-600 mx-1">
                        <?php echo $i; ?>
                    </a>
                <?php endfor; ?>

                <?php if ($page < $totalPages): ?>
                    <a href="?page=<?php echo $page + 1; ?>" class="px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600 mx-1">Next</a>
                <?php endif; ?>
            </div>
